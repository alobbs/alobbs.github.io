...
date: 2026-09-20 23:58:15.675873
draft: false
categories: [ "Briefing" ]
params:
  math: false
...
Linux 7.3-rc4 landed with broad fixes, some flagged by LLMs, plus x86 patches blocking Granite Rapids microcode and fixing FRED crashes on Panther Lake. Gzip 1.15 fixed long-standing bugs, including corruption and an LZH overflow. KDE set two-year goals: enterprise features, better docs, and next-gen styling. A Windows update turned months of inactivity into a seven-hour hostage ordeal.
...
An article alleges that Google AI Studio’s “Delete permanently” feature does not actually purge chat data from Google’s backend, but only moves a local JSON pointer to Google Drive Trash, allowing chats to be recovered. The author says they reported the issue to Google’s VRP and were permanently banned within 60 seconds without human review, while the ticket was closed as “Intended Behavior.” The article also claims the evidence was forwarded to privacy regulators over potential GDPR violations.

- [Google AI Studio fakes data deletion. VRP auto-banned me in 60s for reporting it](https://medium.com/@istokovicsgyorgy79/google-ai-studio-fakes-data-deletion-vrp-auto-banned-me-in-60s-for-reporting-it-ea68e06f9bc2) — *medium.com*
...
KDE is celebrating its 30th anniversary, and an Akademy talk has proposed an AI-native desktop. The proposal imagines Plasma assembling itself around a personal model of each user.
...
Over the past year, w64devkit gained Peter0x44 as co-maintainer, whose contributions improved the project and were partly upstreamed. Release packaging is now code-signed, MSYS2 adopted its aas-sign tool, builds are automated via GitHub Actions, and releases are immutable with attestations. The x64 release is now a multilib toolchain that can also compile 32-bit Windows programs, and GCC can be invoked without adding w64devkit’s bin directory to PATH.
