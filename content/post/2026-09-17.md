...
date: 2026-09-18 04:22:15.951940
draft: false
categories: [ "Briefing" ]
params:
  math: false
...
Australia's Telstra suffered a major outage after a GPS receiver returned from maintenance and spread the wrong time, thinking it was 2006, disrupting calls, trains, payments and emergency services; a review warns of reliance on precise time. Rust maintainers warn of targeted social-engineering attacks linked to an arrayref supply-chain compromise. TypeSafe AI unveiled Jev, a fast type-safe structured-output model.
...
TypeSafe AI, founded by Diogo Almeida, announced System One Models, a new class of frontier models designed to make fast, structured decisions directly usable by software. Its first public model, Jev, is available in early access and reportedly matches existing LLMs on System One tasks while being two orders of magnitude faster and more efficient. Jev forgoes text generation in favor of type-safe structured outputs with calibrated probabilities, avoiding hallucinations.
...
### Anthropic redesigns Claude Projects: describe work once, Claude runs parallel threads in Claude Code

Anthropic has redesigned Claude projects so users can describe work in a single conversation and have Claude scope, delegate, and coordinate it across parallel threads, review outputs, and assemble the finished result. The updated experience is rolling out in beta in Claude Code, starting with select Claude Pro and Max subscribers using cloud sessions who have no existing web or desktop projects, with broader access planned over the coming weeks.

- [Anthropic redesigns Claude Projects: describe work once, Claude runs parallel threads in Claude Code](https://claude.com/blog/projects-redesigned) — *claude.com*
...
Rust Coreutils 0.12 has been released, days after GNU Coreutils 9.12, with a focus on compatibility fixes requested by Ubuntu ahead of its planned 100% transition to Rust Coreutils in Ubuntu 26.10. The release fixes issues including rm segfaults on deeply nested directories, data loss in parallel install -D, du support on 32-bit architectures, and cp -R name collisions, while removing the last C dependency (expr) and improving security, cross-platform support, and performance. It still passes about 95% of the GNU Coreutils test suite.

- [Rust Coreutils 0.12 Released With Fixes Sought By Ubuntu](https://www.phoronix.com/news/Rust-Coreutils-0.12-Released) — *phoronix.com*
...
OpenAI released a framework for identifying when its models misalign with human goals, a move described as a tactical effort to shape AI-safety debate and preempt stricter global regulation. The framework includes six internal case studies with no real-user impact, while coverage split between Japanese technical/engineering focus and Western safety/ethics focus. Critics warn that such self-regulation could normalize harmful AI behavior and shield intellectual property, limiting independent scrutiny as other firms and regulators respond.

- [OpenAI's Misalignment Framework: A Tactical Bid to Preempt Global AI Governance](https://asiaai.fyi/openai-misalignment-framework-global-governance/) — *asiaai.fyi*
